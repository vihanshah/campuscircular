 ...
 * configures with `resolve_model_ids: "omit"` — which is every non-the agent runtime, see
 ...
 *   - a per-agent `model_overrides` pin naming a known alias (or a full the agent id that
...
function resolveTierFromConfig(config, agentType) {
    ...
    if (profileTier && profileTier !== 'inherit') {
        ...
        if (policyModel) {
            ...
            // map — including every non-the agent runtime, where resolveModelInternal
