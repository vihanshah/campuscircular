...
import { Repeat, Clock, CheckCircle2, AlertTriangle, Send, Search, Filter, ShieldCheck, FileText, X } from "lucide-react";
import { loadAppStore, saveAppStore, SharedLoan } from "@/lib/appStore";

export const AdminExchangesTab: React.FC = () => {
  const [store, setStore] = useState(() => loadAppStore());
  const [searchQuery, setSearchQuery] = useState("");
  const [statusFilter, setStatusFilter] = useState<"ALL" | "ACTIVE" | "DUE_SOON" | "RETURNED">("ALL");
  ...
  const handleReturnLoan = (loanId: string, itemTitle: string) => {
    if (confirm(`Mark loan for "${itemTitle}" as COMPLETED & RETURNED?`)) {
  ...
  const handleSendReminderSMS = (borrowerName: string) => {
    alert(`⚡ Twilio Automated SMS Reminder dispatched to ${borrowerName}'s registered phone number!`);
  };
    ...
    <div className="space-y-6 text-white font-sans">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-[#1A1A24] border border-white/10 p-5 rounded-3xl space-y-1">
          <div className="text-[10px] font-mono uppercase text-white/50">Total Active Loans</div>
          <div className="text-3xl font-black text-[#00F2FE]">{allLoans.filter((l) => l.status !== "RETURNED").length}</div>
          <div className="text-[10px] text-white/60">Live across 10 campus departments</div>
        </div>

        <div className="bg-[#1A1A24] border border-white/10 p-5 rounded-3xl space-y-1">
          <div className="text-[10px] font-mono uppercase text-white/50">Due Soon (24-48h)</div>
          <div className="text-3xl font-black text-[#FFD928]">{allLoans.filter((l) => l.status === "DUE_SOON").length}</div>
          <div className="text-[10px] text-amber-300">Automated SMS Reminders queued</div>
        </div>

        <div className="bg-[#1A1A24] border border-white/10 p-5 rounded-3xl space-y-1">
          <div className="text-[10px] font-mono uppercase text-white/50">Completed Exchanges</div>
          <div className="text-3xl font-black text-[#34D399]">142</div>
          <div className="text-[10px] text-[#34D399]">100% Verified Escrow Return</div>
        </div>

        <div className="bg-[#1A1A24] border border-white/10 p-5 rounded-3xl space-y-1">
          <div className="text-[10px] font-mono uppercase text-white/50">On-Time Return Rate</div>
          <div className="text-3xl font-black text-[#B92CFF]">98.4%</div>
          <div className="text-[10px] text-white/60">Trusted student community</div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#1A1A24] p-6 rounded-3xl border border-white/10 shadow-lg">
        <div>
          <div className="flex items-center gap-2 text-xs font-black uppercase text-[#B92CFF] mb-1">
            <Repeat className="w-4 h-4" />
            <span>Campus Loans & Exchange Registry</span>
          </div>
          <h2 className="text-2xl font-black text-white">Live Exchanges Directory ({filteredLoans.length})</h2>
          <p className="text-xs font-semibold text-white/60 mt-0.5">
            Admin oversight for all active borrowing contracts, handover agreements, and return schedules.
          </p>
        </div>

        <div className="flex items-center gap-2">
          {(["ALL", "ACTIVE", "DUE_SOON", "RETURNED"] as const).map((st) => (
            <button
              key={st}
              onClick={() => setStatusFilter(st)}
              className={`px-3 py-1.5 rounded-xl text-xs font-extrabold uppercase transition-all ${
                statusFilter === st
                  ? "bg-[#B92CFF] text-white shadow-md"
                  : "bg-white/05 text-white/70 hover:bg-white/10"
              }`}
            >
              {st === "DUE_SOON" ? "Due Soon" : st}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="bg-[#1A1A24] p-4 rounded-2xl border border-white/10 shadow-sm flex items-center justify-between gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search by equipment name, borrower (CC1003), or owner (CC1007)..."
            className="w-full pl-10 pr-4 py-2 bg-[#0F0F14] border border-white/10 rounded-xl text-xs font-semibold text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-[#B92CFF]"
          />
        </div>

        <div className="text-xs font-mono text-white/50 shrink-0">
          Showing {filteredLoans.length} active exchanges
        </div>
      </div>
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-[10px] font-mono font-black uppercase ${
                        loan.status === "RETURNED"
                          ...
                          : "bg-[#B92CFF]/20 text-[#B92CFF] border border-[#B92CFF]/40"
                      }`}
                    >
        ...
        <div className="fixed inset-0 z-50 bg-[#0F0F14]/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#1A1A24] border border-white/15 rounded-3xl p-6 sm:p-8 max-w-lg w-full space-y-5 text-white relative shadow-2xl">
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-[#B92CFF] text-white flex items-center justify-center font-black">
                  📄
                </div>
                <div>
                  <h3 className="text-lg font-black text-white">Borrowing Contract</h3>
...
export default AdminExchangesTab;
