...
interface SocialCardsProps {
  ...
  onHoverMood?: (mood: any | null) => void;
  activeMood?: string;
}
...
export default function SocialCards({ cards, onSelectMood, onHoverMood, activeMood }: SocialCardsProps) {
  ...
  useEffect(() => {
    ...
    const enterHandlers = visibleEntries.map(({ el, slot, cardIndex }) => {
      const handler = () => {
        ...
        if (activeSlot !== slot) {
          ...
          if (onHoverMood && cards[cardIndex]?.mood) {
            onHoverMood(cards[cardIndex].mood);
          }
        }
      };
    ...
    const onMouseLeave = () => {
      ...
      leaveTimer = setTimeout(() => {
        ...
        if (onHoverMood) onHoverMood(null);
      }, 50);
    };
  ...
  }, [centerIndex, totalCards, getVisibleMap, needsPagination, onHoverMood, cards]);
  ...
  return (
    <section className="flex flex-col items-center w-full py-2 px-2 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]">
          {cards.map((card, index) => {
            const isSelected = activeMood && card.mood === activeMood;
            const cardBody = (
              <div
                onClick={() => {
                  if (onSelectMood && card.mood) {
                    onSelectMood(card.mood);
                  }
                }}
                className={`relative w-20 h-20 md:w-24 md:h-24 rounded-full flex flex-col items-center justify-center cursor-pointer border backdrop-blur-xl shadow-2xl transition-all duration-300 ${
                  isSelected
                    ? "border-white/90 ring-4 ring-white/60 scale-110 shadow-[0_0_30px_rgba(255,255,255,0.5)]"
                    : "border-white/20 hover:border-white/60 hover:scale-105"
                }`}
                style={{
                  background: card.color
                    ? `radial-gradient(circle at 35% 35%, ${card.color}, ${card.color}aa)`
                    : "rgba(255, 255, 255, 0.15)"
                }}
              >
                {/* Content inside circular emoji badge */}
                {card.content ? (
                  card.content
                ) : (
            ...
            return (
              <div key={index} className="fan-card absolute transition-transform">
                {cardBody}
              </div>
            );
          })}
